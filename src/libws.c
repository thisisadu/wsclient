...
int ws_init(ws_t *ws, char* ca_path, ws_base_t ws_base)
{
	struct ws_s *w = NULL;
...
struct event_base *ws_get_event_base(ws_base_t base)
{
  return base->ev_base;
}
...
int ws_add_header(ws_t ws, const char *header, const char *value)
{
	...
	assert(value);
  char buf[1024] = {0};
  sprintf(buf,"%s:%s",header,value);

	if (!(ws->headers = (char **)_ws_realloc(ws->headers,
                                                (ws->num_headers + 1) * sizeof(char *))))
    {
      LIBWS_LOG(LIBWS_ERR, "Out of memory!");
      return -1;
    }

	ws->headers[ws->num_headers] = _ws_strdup(buf);
	ws->num_headers++;
...
int ws_clear_headers(ws_t ws)
{
	...
	for (i = 0; i < ws->num_headers; i++)
    {
      if (ws->headers[i])
        _ws_free(ws->headers[i]);
    }
...
int ws_add_subprotocol(ws_t ws, const char *subprotocol)
{
	...
	ws->subprotocols[ws->num_subprotocols] = _ws_strdup(subprotocol);
	ws->num_subprotocols++;
