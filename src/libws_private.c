...
static char *mstrdup(char *str){
  if(str == NULL)return NULL;
  int len = strlen(str);
  char *str2 = calloc(1,len+1);
...
static void _ws_error_event(struct bufferevent *bev, short events, void *ptr)
{
  ...
  /*     ws->close_cb(ws, ws->server_close_status,err_msg, strlen(err_msg), ws->close_arg); */
  /*   } */
  /* } */

  if(ws->connect_state >= WS_CONNECT_STATE_SENT_REQ){
    err = WS_CONNECT_BREAK;
    err_msg = "connect break";
  }else{
    err = WS_CONNECT_FAILED;
